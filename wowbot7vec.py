...
from load_v8_combined_wownet7vec import get_configured_model
...
def process_img(img): 
    ...
    #replay video
    cv2.imshow('WoW replay', cv2.cvtColor(img.numpy()[0][0], cv2.COLOR_BGR2RGB))
...
while(True):
    ...
    #pred_keys = np.round(pred)
    pred_keys = np.where(pred > 0.5, 1, 0)


    os.system('cls')
    print(f'{pred} SUM: {np.sum(pred)}')
    print(f'{pred_keys}')
